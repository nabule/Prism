...
真实 LLM 整理、附件解析、RAG、自动文档和备份恢复属于后续阶段。
...
该草案用于 P2 早期验证标签治理和任务结构，不会调用 LLM。内容明确时，worker 会基于草案创建一条确定性 Markdown AI 整理 memo，内容包含 `#系统/AI整理`、来源 memo、摘要、标签状态和原文摘录，并在 job `result.ai_summary_memo_uid` 返回新 memo uid。
