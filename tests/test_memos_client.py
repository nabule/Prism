...
async def test_memos_client_reads_memo_and_creates_comment(monkeypatch):
    ...
    async def list_memos(pageSize: int, pageToken: str | None = None, filter: str | None = None):
        return {
            "memos": [{"name": "memos/memo-demo", "content": "hello"}],
            "nextPageToken": "",
            "pageSize": pageSize,
            "pageToken": pageToken,
            "filter": filter,
    ...
    assert await client.get_memo("memo-demo") == {"name": "memos/memo-demo", "content": "hello"}
    ...
    assert await client.update_memo_content("memo-demo", "# 回写标题") == {
        "name": "memos/memo-demo",
        "content": "# 回写标题",
    }
    assert await client.create_comment("memo-demo", "comment") == {
        "name": "memos/memo-demo/comments/1",
        "content": "comment",
    }
    assert await client.list_memo_relations("memo-demo") == {"relations": [], "nextPageToken": ""}
    assert await client.upsert_memo_reference_relation(
        source_memo_uid="memo-demo",
